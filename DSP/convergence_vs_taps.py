...
def plot_convergence_vs_num_taps():
    ...
    for num_taps in range(11,201,10):
        converged_sample_sum = 0
        for iter in range(NUM_AVG):
            E_out, stats = cma_utils.mcma_python(E_after_pmd,num_taps,mu_CMA=1e-5)
            pxx, pxy, pyx, pyy, cma_error= stats['pxx'], stats['pyx'], stats['pyx'], stats['pyy'], stats['cma_error']
            ...
            converged_sample_sum += find_convergence_backward(smoothed_log_errors)
    ...
    plt.ylabel("Converged Samples")
    plt.savefig("converged_samples_vs_num_taps.png")
    #plt.show()
